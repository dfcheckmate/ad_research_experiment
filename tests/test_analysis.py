...
def test_classify_request_role():
    ...
    row_ad = pd.Series(
        {"ad_url": "https://pubads.g.doubleclick.net/", "ad_domain": "doubleclick.net"}
    )
...
def test_classify_platform_class():
    ...
    row_google = pd.Series(
        {"ad_domain": "googlesyndication.com", "request_role": "ad loader script"}
    )
    assert classify_platform_class(row_google) == "google display stack"

    row_outbrain = pd.Series(
        {"ad_domain": "outbrain.com", "request_role": "native recommendation widget"}
    )
...
def test_classify_final_taxonomy():
    ...
    row_identity = pd.Series(
        {"request_role": "identity sync", "platform_class": "identity / sync layer"}
    )
    assert classify_final_taxonomy(row_identity) == "identity sync"

    row_display = pd.Series(
        {"request_role": "ad serving request", "platform_class": "google display stack"}
    )
...
def test_volume_hypothesis_with_sample_data():
    ...
    data = {
        ...
        "zip_condition": [
            "orem_ut" if i % 3 == 0 else "boston_ma" if i % 3 == 1 else "nyc_ny"
            for i in range(100)
        ],
        "ad_shown": [1] * 100,
    }
