...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["source_type"] = df.get("source_type", pd.Series([None] * len(df))).fillna("page_load")
    df["intent_profile"] = df.get("intent_profile", pd.Series([None] * len(df))).fillna("none")
    df["query_topic"] = df.get("query_topic", pd.Series([None] * len(df))).fillna("none")
    df["search_query"] = df.get("search_query", pd.Series([None] * len(df))).fillna("")
    df["inferred_topic"] = df.get("inferred_topic", pd.Series([None] * len(df))).fillna("uncategorized")
    ...
    uncategorized = df["inferred_topic"].str.lower().str.contains("uncategorized|unknown|other", na=False)
    df.loc[uncategorized, "clean_topic_label"] = "uncategorized"

    if "landing_domain" in df.columns:
        df.loc[df["landing_domain"].str.contains("google", na=False), "clean_topic_label"] = "google_search"
        df.loc[df["landing_domain"].str.contains("youtube", na=False), "clean_topic_label"] = "youtube"

    df["platform_class"] = "other"
    df.loc[df["ad_network"].str.contains("google", na=False), "platform_class"] = "google"
    df.loc[df["ad_network"].str.contains("amazon", na=False), "platform_class"] = "amazon"
    df.loc[df["ad_network"].str.contains("criteo", na=False), "platform_class"] = "criteo"
    df.loc[df["ad_network"].str.contains("taboola", na=False), "platform_class"] = "taboola"
    df.loc[df["ad_network"].str.contains("outbrain", na=False), "platform_class"] = "outbrain"

    df["request_role"] = "other"
    df.loc[df["source_type"] == "google_search_ad", "request_role"] = "search_ad"
    df.loc[df["source_type"].isin(["page_load", "iframe", "xhr"]), "request_role"] = "display_ad"
    ...
    if "inferred_topic" in df.columns and "intent_profile" in df.columns:
        ...
        for intent in df["intent_profile"].unique():
            for zip_condition in df["zip_condition"].unique():
                sub = df[(df["intent_profile"] == intent) & (df["zip_condition"] == zip_condition)]
                if len(sub) == 0:
                    continue
...
def final_taxonomy_summary(df: pd.DataFrame, output_dir: str) -> None:
    ...
    if "intent_profile" in df.columns:
        ...
        table = pd.crosstab([df["zip_condition"], df["intent_profile"]], df["final_taxonomy"])
        print(table.to_string())

    csv_path = os.path.join(output_dir, "final_taxonomy.csv")
    if "intent_profile" in df.columns:
        out_df = pd.crosstab([df["zip_condition"], df["intent_profile"]], df["final_taxonomy"])
    else:
...
def cell_comparison_plot(df: pd.DataFrame, output_dir: str) -> None:
    ...
    pivot = pd.crosstab([df["intent_profile"], df["zip_condition"]], df["platform_class"])
...
def chi_square_test(df: pd.DataFrame) -> None:
    ...
    if p < 0.05:
        log_result("→ Ad domain distribution differs significantly across ZIP conditions")
    else:
...
def per_domain_odds(df: pd.DataFrame, output_dir: str) -> None:
    ...
    others = [x for x in df["zip_condition"].unique() if x != ref]
    ...
    for domain in df["ad_domain"].unique()[:20]:
        ...
        sub["zip_ref"] = sub["zip_condition"].apply(lambda x: "other" if x != ref else ref)
...
def test_volume_hypothesis(df: pd.DataFrame) -> None:
    ...
    trial_counts = df.groupby(["trial_id", "zip_condition", "intent_profile"]).size().reset_index(name="ad_count")
    ...
    model = smf.glm("ad_count ~ zip_condition", data=trial_counts, family=sm.families.Poisson()).fit()
    ...
    if any(model.pvalues.drop("Intercept") < 0.05):
        log_result("→ At least one identity level significantly predicts ad volume (α=0.05)")
    else:
...
def inferred_topic_summary(df: pd.DataFrame, output_dir: str) -> None:
    ...
    sub = df.copy()
    sub = sub[sub["inferred_topic"].str.lower().str.contains("uncategorized|unknown|other", na=False) == False]
...
def personalization_analysis(df: pd.DataFrame, output_dir: str) -> None:
    ...
    if "ad_domain" in df.columns:
        ...
        sub = df[df["ad_domain"].isin(df["ad_domain"].value_counts().head(5).index)].copy()
        sub = sub.copy()
        ...
        try:
            ...
            log_result("[personalization] Interaction model failed — insufficient data or separation")
...
def ranking_analysis(df: pd.DataFrame, output_dir: str) -> None:
    ...
    log_result("       Quartile-based ordering (Q1/Q2/Q3/Q4) within intent_profile blocks.")
    ...
    if "observed_at" not in df.columns or df["observed_at"].isna().all():
        log_result("[ranking] No usable observed_at timestamps — skipping order analysis.")
        return
    ...
    if "intent_profile" in sub.columns:
        sub["within_block_rank"] = sub.groupby(["trial_id", "intent_profile"])["observed_at"].rank(method="first")
        sub["block_size"] = sub.groupby(["trial_id", "intent_profile"])["within_block_rank"].transform("count")
        sub["quartile_position"] = sub["within_block_rank"] / sub["block_size"]
        sub["order_quartile"] = pd.qcut(sub["quartile_position"], q=4, labels=["Q1", "Q2", "Q3", "Q4"], duplicates="drop")
    else:
        sub["within_trial_rank"] = sub.groupby("trial_id")["observed_at"].rank(method="first")
    ...
    if "intent_profile" in sub.columns:
        sub["prev_observed_at"] = sub.groupby(["trial_id", "intent_profile"])["observed_at"].shift(1)
    else:
        sub["prev_observed_at"] = sub.groupby("trial_id")["observed_at"].shift(1)
    sub["time_delta_sec"] = (sub["observed_at"] - sub["prev_observed_at"]).dt.total_seconds()

    # Position estimation stub (for future explicit rank capture)
    sub["estimated_position"] = sub["within_block_rank"] if "intent_profile" in sub.columns else sub["within_trial_rank"]
    ...
    for q in ["Q1", "Q2", "Q3", "Q4"]:
        ...
        entropy_by_quartile.append({"quartile": q, "n_ads": len(q_data), "unique_domains": len(domain_counts), "shannon_entropy": entropy})
    ...
    if entropy_by_quartile:
        ...
        for _, row in entropy_df.iterrows():
            print(f"{row['quartile']:8s} | {row['n_ads']:5d} | {row['unique_domains']:14d} | {row['shannon_entropy']:.4f}")
    ...
    if contingency_quartile.shape[0] >= 2 and contingency_quartile.shape[1] >= 2:
        try:
            ...
            if p < 0.05:
                log_result("→ Quartile distribution differs significantly across proxy identities")
            else:
                log_result("→ No significant difference in quartile distribution across identities")
        except Exception as e:
    ...
    export_cols = ["trial_id", "zip_condition", "order_quartile", "estimated_position", "time_delta_sec", "quartile_position"]
    if "intent_profile" in sub.columns:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Causal analysis of ad-targeting experiment")
    parser.add_argument("--output", default="out/results", help="Directory for plots and CSV")
    parser.add_argument(
        "--hypothesis",
        default="all",
